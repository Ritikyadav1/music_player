...
const buttonElement = document.getElementById('li');
...
// Generate a random index within the range of the songs array length
const randomIndex = Math.floor(Math.random() * songs.length);

// Get the randomly selected song object
...
const loadSong = (songs) =>{
    ...
    // music.src = 'music/' + randomSong.title + '.mp3';
...
list_up.addEventListener('click', () => {
    if (!rotate) {
        ...
        list_box.style.height = '93%';
        list.style.display = 'inline';
        playlistContainer.style.display = 'block'; // Show the playlist
        // Add playlist items
        for (let i = 0; i < songs.length; i++) {
            const x = songs[i].title;
            const y = songs[i].artist;
        ...
        list_box.style.height = '40px';
        list.style.display = 'none';
        playlistContainer.style.display = 'none'; // Hide the playlist
        playlistList.innerHTML = ''; // Clear playlist items
        rotate = false;
    }
});
